@extends('layout')
@section('principal')
  <h2>Bem vindo administrador! {{Auth::user()->name}} </h2>
...
<script>
    ...
    type: 'bar',
    data: {
        labels: {!! json_encode($destinosFrequentes->pluck('destino')) !!},
        datasets: [{
...
<script>
    ...
    @foreach($viagensProximas as $v)
        L.marker([{{ $v->latitude }}, {{ $v->longitude }}])
            .addTo(mapa)
            .bindPopup('<strong>{{ $v->destino }}</strong><br>{{ $v->cliente->nome }}');
    @endforeach
</script>
@endsection